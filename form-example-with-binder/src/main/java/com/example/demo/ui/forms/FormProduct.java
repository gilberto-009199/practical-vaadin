...
public class FormProduct extends Composite<Component> {
	...
	public FormProduct withProduct(Product product) {
		values = new Binder<>(Product.class);
		
		values.forField(name)
		.asRequired()
		.withValidator(value -> value.length() > 2, "Minimo de 3 caracters")
		.bind(
			Product::getName,
			Product::setName
		);
		
		values.forField(code)
		.asRequired()
		.withValidator(value -> value.length() > 2, "Minimo de 3 caracters")
		.bind(
			Product::getCode, 
		...
		values.forField(validate)
		.asRequired("Data de validade")
		.withValidator(
			value -> value.isAfter(LocalDate.now()),
			"Data de validade tem que ser superior ao dia atual"
		)
		.bind(
			Product::getValidate,
			Product::setValidate
		...
		values.forField(avaliable)
		.bind(
			(prod) -> !prod.isAvaliable(),
			(prod, booleanValue) -> prod.setAvaliable(!booleanValue)
		);
